...
final readonly class GridView
{
    ...
    /**
     * What the whole grid answers, in the five words the sketch puts above it.
     *
     * The tab counters answer «what does this role grant» per tab; this answers
     * it for the role, and adds the three facts a counter cannot carry: how many
     * cells are narrowed, how many end on a date, and how many arrive through a
     * role this one inherits. Those three are the whole of what 3.0 added, and
     * without a line saying so they are only visible by hovering every cell.
     *
     * Counted from what the cells ANSWER and never from what was written
     * (§6.11): a role holding the wildcard has written no cell and answers them
     * all, and counting the writes would tally the most dangerous role in the
     * installation at zero.
     *
     * @return array{granted: int, forbidden: int, narrowed: int, ending: int, inherited: int, total: int}
     */
        ...
        foreach ($this->tabs as $tab) {
            foreach ($tab->rows as $row) {
                foreach ($row->allCells() as $cell) {
                    ...
                    if ($cell->until instanceof CarbonImmutable) {
                        $ending++;
                    }
