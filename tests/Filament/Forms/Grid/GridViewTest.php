...
test('the grid says what it answers as a whole, and counts what a tab counter cannot', function (): void {
    $role = makeRole();

    // Sobre el rol y no sobre `Post`: el panel `test` registra los dos recursos
    // del plugin y ningún otro, así que una concesión sobre `Post` no cae en
    // ninguna celda de este catálogo y el recuento saldría cero por el motivo
    // equivocado.
    Warden::allow($role)->to('viewAny', roleClass());
    Warden::allow($role)->until(now()->addWeeks(2))->to('view', roleClass());
    Warden::forbid($role)->to('update', roleClass());

    $catalog = Catalog::for(Filament::getPanel('test'));

    $held = RoleGrants::of($role, $catalog);

    // El tercer argumento es lo que el navegador tiene en pantalla, y es de
    // donde salen las posturas: `RoleState` lleva el estrechamiento, las fechas
    // y la herencia, no las posturas. Sin él la rejilla se dibuja entera en
    // abstención y el recuento sale cero por el motivo equivocado.
    $grid = GridView::for($catalog, $held, $held->stances);

    $tally = $grid->tally();

    // Counted from what the cells ANSWER and never from what was written: a role
    // holding the wildcard has written no cell and answers them all, so counting
    // the writes would tally the most dangerous role in the installation at zero
    // (§6.11). The three figures a tab counter cannot carry are the point —
    // narrowed, ending, inherited — because without this line they are only
    // visible by hovering every cell.
    expect($tally['granted'])->toBe(2)
        ->and($tally['forbidden'])->toBe(1)
        ->and($tally['ending'])->toBe(1)
        ->and($tally['total'])->toBeGreaterThan(2);
});
