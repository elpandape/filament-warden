...
return [
    ...
    'resources' => [
        ...
        'permissions' => [
            ...
            'grant' => [
                'label' => 'Grant to an account',
                'description' => 'A direct grant, with no role in between. It is the same catalogue row, so editing it later changes what this account holds too.',
                'account' => 'Account',
                'polarity' => 'What to write',
                'granted' => 'Granted',
                'forbidden' => 'Forbidden',
                'until' => 'Ends on',
                'until_help' => 'Leave it empty for a grant with no end. Past the date warden stops reading it on its own, with no command run — `warden:clean --expired` only removes the row.',
                'forbidden_help' => 'A prohibition beats every grant the account\'s roles carry, and it does not expire: warden refuses a date on one. It is lifted with unforbid(), not by waiting.',
                'done' => 'Written',
                'done_granted' => ':account now holds this permission directly.',
                'done_forbidden' => ':account is now explicitly forbidden this permission. Nothing its roles carry can grant it back.',
            ],
            'probe' => [
                'label' => 'Test it',
                ...
                'description' => 'Pick an account and, if the permission has a model, a row. Warden answers the verdict and the cause. It is explain() put on screen: it always goes to the database, never to the cache — so it can disagree with the panel, and when it does, something failed to invalidate.',
                'answer' => 'What the store answered',
                'via' => 'How it was reached',
                'until' => 'Until',
                'reach' => 'How far it falls',
            ],
        ],
    ],
    ...
    'explain' => [
        ...
        'inherited' => 'This cell is answered by :role, which this role inherits. Changing it here writes a rule of this role’s own on top; :role keeps theirs.',
    ...
    'console' => [
        'audit' => [
            ...
            'unwalkable' => 'Models only a relation manager reaches. Reaching one means running the relationship, which is not safe to do here, so they are named instead. A relation manager declaring `$relatedResource` is walked for free; one that cannot declare it stays on this list for good — this package\'s own `RolesRelationManager` and `PermissionsRelationManager` are two, deliberately. `catalog.models` is what puts the model in the catalogue; it does not clear the line.',
    ...
    'grid' => [
        ...
        'until' => [
            'label' => 'Ends on',
            'forbidden' => 'A prohibition does not expire. Lift it when it should stop applying.',
            'unwritten' => 'There is no rule of this role’s own here to give a date to.',
            'off' => 'Dates are not set from this screen in this installation. A grant that already ends still ends: warden honours the date whatever this screen may do with it.',
        ],
