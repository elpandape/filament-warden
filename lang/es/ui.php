...
return [
    ...
    'console' => [
        'audit' => [
            ...
            'unwalkable' => 'Modelos a los que solo llega un relation manager. Llegar a uno exige ejecutar la relación, que aquí no es seguro hacer, así que se nombran en vez de resolverse. Un relation manager que declara `$relatedResource` se recorre gratis; uno que no puede declararlo se queda en esta lista para siempre —el `RolesRelationManager` y el `PermissionsRelationManager` de este paquete son dos, a propósito—. `catalog.models` es lo que mete el modelo en el catálogo; no quita la línea.',
            ...
            'expired' => 'Concesiones y asignaciones de rol pasadas de fecha. No es un defecto ni algo que alguien hiciera: llegó una fecha y warden dejó de leer la fila sin tocarla. Pero no son inertes — una fila muerta sigue a su permiso o a su rol por una clave ajena, así que sigue bloqueando un borrado con `roles.delete => \'unassigned\'` y sigue cerrando un nombre con `permissions.update => \'loose\'`, las dos cosas a propósito. `warden:clean --expired` las borra.',
    ...
    'grid' => [
        ...
        'until' => [
            ...
            'off' => 'En esta instalación las fechas no se ponen desde esta pantalla. Una concesión que ya caduca sigue caducando: warden honra la fecha haga lo que haga esta pantalla.',
        ],
